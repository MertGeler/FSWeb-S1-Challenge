...
console.log("1. fenomenin Profil Adı" + " " + fenomenler[0].profile);

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log("3. fenomenin Takipçi Sayısı" + " " + fenomenler[2].followers);
...
function indekseGoreFenomen(fenomenler, indeks) {
  ...
  const sonuc = `${indeks}. indekste bulunan fenomen: ${fenomen.profile}`;
...
function enFenomenler(fenomenDizisi) {
  const enFenomenlerDizi = fenomenDizisi.filter(
    (fenomen) => fenomen.followers > 100000000
  );
  const enFenomenlerAdları = enFenomenlerDizi.map((fenomen) => fenomen.profile);
  return enFenomenlerAdları;
}
...
function fenomenGonderimSayisi(fenomenDizisi, profilAdı) {
  const fenomen = fenomenDizisi.find(
    (fenomen) => fenomen.profile === profilAdı
  );
  if (!fenomen) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenDizisi, platformAdı) {
  let enCokGonderiSayisi = -1;
  let enCokGonderiYapanFenomen = "";

  for (const fenomen of fenomenDizisi) {
    if (fenomen.platform === platformAdı && fenomen.posts !== "NA") {
      const gonderiSayisi = parseInt(fenomen.posts, 10);
